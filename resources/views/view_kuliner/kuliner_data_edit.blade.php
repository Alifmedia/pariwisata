...
@section('content')
  <div id="kuliner-add">
    ...
    <div class="container my-4">
      ...
      <div class="card">
        <div class="card-body">
          <h3 class="card-title">Edit Kuliner</h3>
          <form action="{{ route('kuliner.update') }}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="row">
              <div class="col-6">
                ...
                <div class="form-group">
                  ...
                  <select class="form-control{{ $errors->has('kategori') ? ' is-invalid' : '' }}" name="kategori" id="kategori" required>
                    ...
                    @foreach ($filter['kategori'] as $kategori)
                      <option value="{{ $kategori->id }}" {{ $data->kuliner_kategori_id == $kategori->id ? 'selected' : '' }}>{{ $kategori->nama }}</option>
                    @endforeach
                  </select>
      ...
      <div class="owl-carousel owl-theme my-4">
        @foreach ($data->kulinerFoto as $key => $foto)
          <div class="item">
            <a href="{{ route('kuliner.foto.delete') }}" class="action-delete-btn" onclick="event.preventDefault();document.getElementById('foto-{{ $key }}').submit();">
              <i class="fa fa-times" aria-hidden="true"></i>
            </a>
            <img src="{{ asset($foto->source) }}" alt="">
          </div>
          <form id="foto-{{ $key }}" action="{{ route('kuliner.foto.delete') }}" method="POST" class="d-none">
            @csrf
