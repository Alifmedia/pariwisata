@extends('view_sanggar_seni.sanggar_seni')

@section('sub-content')
  <div id="data">
    @include('template.flash')
    <form id="search-form" action="{{ route('sanggar_seni') }}" method="get">
      <div class="search">
        ...
        <div class="filter__input__sub">
          <div class="form-group">
            ...
            <select class="form-control" id="filter1" name="kategori">
              ...
              @foreach ($filter['jenis'] as $jenis)
                <option value="{{ $jenis->id }}" {{ app('request')->input('jenis') == $jenis->id ? 'selected' : '' }}>{{ $jenis->nama }}</option>
              @endforeach
            </select>
          </div>
          <div class="form-group">
    ...
    {{-- Table --}}
    <br>
    <form id="delete-form" action="{{ route('sanggar_seni.delete') }}" method="POST">
      @csrf
      <button type="submit" class="btn btn-danger" name="button">
        <i class="fa fa-trash" aria-hidden="true"></i>&nbsp;
        Hapus
      </button>
      <a class="btn btn-primary" href="{{ route('sanggar_seni.create') }}">
            ...
            <tbody>
              @foreach ($datas as $key => $data)
                <tr>
                  ...
                  <td>{{ $data->no_hp }}</td>
                  <td>
                    <a href="{{ route('sanggar_seni.show', $data->id) }}">
                      <i class="fa fa-ellipsis-h" aria-hidden="true"></i>
                    </a>
                  </td>
                  <td>
                    <a href="{{ route('sanggar_seni.edit', $data->id) }}">
                      <i class="fa fa-pencil-square-o" aria-hidden="true"></i>
                    </a>
                  </td>
                </tr>
