...
@section('content')
  <div id="money-changer-add">
    ...
    <div class="container my-4">
      ...
      <div class="card">
        <div class="card-body">
          <h3 class="card-title">Edit Money Changer</h3>
          <form action="{{ route('money_changer.update') }}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="row">
              <div class="col-6">
      ...
      <div class="owl-carousel owl-theme my-4">
        @foreach ($data->moneyChangerFoto as $key => $foto)
          <div class="item">
            <a href="{{ route('money_changer.foto.delete') }}" class="action-delete-btn" onclick="event.preventDefault();document.getElementById('foto-{{ $key }}').submit();">
              <i class="fa fa-times" aria-hidden="true"></i>
            </a>
            <img src="{{ asset($foto->source) }}" alt="">
          </div>
          <form id="foto-{{ $key }}" action="{{ route('money_changer.foto.delete') }}" method="POST" class="d-none">
            @csrf
